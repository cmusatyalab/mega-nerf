...
def _get_mask_opts() -> Namespace:
    ...
    parser.add_argument('--depth_tracks', type=str, nargs='+', required=True)
...
def main(hparams: Namespace) -> None:
    ...
    output_base_path = Path(hparams.output_basedir)
    depth_tracks = hparams.depth_tracks
    output_paths = [output_base_path / 'mask_rgb'] + [output_base_path / f'mask_depth_{track}' for track in depth_tracks]
    track_names = ['rgb'] + [f'depth_{depth_track}' for depth_track in depth_tracks]
    ...
    if 'RANK' in os.environ:
        ...
        if rank == 0:
            [output_path.mkdir(parents=True, exist_ok=hparams.resume) for output_path in output_paths]
        dist.barrier()
    ...
    metadata_rgb_paths = list((dataset_path / 'train' / 'metadata_rgb').iterdir()) \
                         + list((dataset_path / 'val' / 'metadata_rgb').iterdir())
    metadata_depth_paths = [list((dataset_path / 'train' / f'metadata_depth_{track}').iterdir()) \
    ...
    min_position = all_camera_positions.min(dim=0)[0]
    max_position = all_camera_positions.max(dim=0)[0]
    ...
    }, output_path / 'params.pt') for output_path in output_paths]
    ...
    if rank == 0 and not hparams.resume:
        for i in range(centroids.shape[0]):
            [(output_path / str(i)).mkdir(parents=True) for output_path in output_paths]
    ...
    for i, track in enumerate(track_names):
        ...
        for subdir in ['train', 'val']:
            metadata_paths = list((dataset_path / subdir / f'metadata_{track}').iterdir())
